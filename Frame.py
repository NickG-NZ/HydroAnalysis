...
class Frame:
    ...
    def __init__(self, ref_frame: 'Frame', pos_x: float, pos_z: float, rot_y: float):
    ...
    def location(self) -> (float, float, float):
        return self._pos_x, self._pos_z, self._rot_y

    def origin_in_datum(self) -> (float, float):
    ...
    def rotation_in_datum(self) -> float:
        rot_y_D = self._ref_frame.rotation_in_datum()

        return rot_y_D + self._rot_y

    def vector_to_frame(self, x: float, z: float, frame: 'Frame') -> (float, float):
        """
    ...
    def vector_from_frame(self, x: float, z: float, frame: 'Frame') -> (float, float):
        """
